...
void AnticheatMgr::BuildReport(Player* player,uint8 reportType)
{
    ...
    if (m_Players[key].GetTotalReports() > sWorld->getIntConfig(CONFIG_ANTICHEAT_REPORTS_INGAME_NOTIFICATION))
    {
        ...
        // NOT A HACKY WAY, BITCH! TITKATA_BG FIXED IT!
        std::string str = "";
        str = "|cffffffff[|cffff8000AC|cffffffff] [|cffff0000" + std::string(player->GetName().c_str()) + "|cffffffff] |cff00ff80Possible cheater!";
        ChatHandler(player->GetSession()).SendGlobalGMSysMessage(str.c_str());
